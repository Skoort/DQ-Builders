...
public class Chunk : MonoBehaviour
{
	...
	private int[,] biomesmap;
	private int bl_biome;
	private int tl_biome;
	private int tr_biome;
	private int br_biome;
	private int grassland_ratio;
	...
	{
		biomesmap = new int[resolution, resolution];
	...
	private int WorldBiomeFunction(float x, float z)
	{
	...
	private int WorldNoiseFunction(int x, int z, int biome)
	{
		if (biome == 0)
		{
			...
			return Mathf.FloorToInt(Mathf.PerlinNoise(x_noise_grass, z_noise_grass) * 50);
		}
		else
		{
			var x_noise_sand = (transform.position.x + x) * 0.065F + -10043;
			var z_noise_sand = (transform.position.z + z) * 0.0652F + -33275;

			return Mathf.FloorToInt(Mathf.PerlinNoise(x_noise_sand, z_noise_sand) * 10);
		}
	}
