...
public class PC_Controller : MonoBehaviour
{
	...
	void Update()
	{
		...
		var move_dir = transform.forward * move_input.y + transform.right * move_input.x;
		...
		y_velocity += dt * Physics.gravity.y * 0.25F;

		controller.Move(move_dir * trans_speed * Time.deltaTime + Vector3.up * y_velocity);
	...
	void PlaceBlock()
	{
		...
		if (hit_info.transform)
		{
			...
			{
				var decoration = Instantiate(removedItem.decorationPrefab, pos_to_place, Quaternion.identity);
